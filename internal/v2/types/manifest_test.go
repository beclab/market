...
// preserves every field declared on ApplicationInfoEntry, including the
// identity scalars (id / appID / version / cfgType / apiVersion) which
// live in dedicated user_applications columns rather than spec and are
// re-injected via EntryScalars on Compose.
func TestRoundTrip(t *testing.T) {
	...
	composed, err := ComposeApplicationInfoEntry(manifest, EntryScalars{
		ID:         original.ID,
		AppID:      original.AppID,
		Version:    original.Version,
		CfgType:    original.CfgType,
