...
func InitStateMachine(dataSender DataSenderInterface, settingsManager *settings.SettingsManager) {
	...
	glog.V(2).Info("Payment state machine initialized")
}
...
func PurchaseApp(userID, appID, sourceID, xForwardedHost string, appInfo *types.AppInfo) (map[string]interface{}, error) {
	...
	if appInfo.AppEntry != nil && appInfo.AppEntry.ID != "" {
		...
		glog.V(3).Infof("PurchaseApp: Using real app ID from AppEntry: %s (URL param was: %s)", realAppID, appID)
	}
	...
	if appInfo.Price != nil && appInfo.Price.Paid != nil {
		if appInfo.Price.Paid.ProductID != "" {
			...
			glog.V(3).Infof("PurchaseApp: Using productID from Price.Paid: %s", productID)
		}
	}
	if productID == "" {
		productID = getProductIDFromAppInfo(appInfo)
		if productID != "" {
			glog.V(3).Infof("PurchaseApp: Using productID from Products: %s", productID)
		}
	}
	if productID == "" {
		// Final fallback: for paid apps without explicit product_id
		productID = realAppID
		glog.V(3).Infof("PurchaseApp: productID not found in price, using real appID as fallback: %s", productID)
	}
	...
	if globalStateMachine != nil {
		...
		if st, err := globalStateMachine.getState(userID, realAppID, productID); err == nil {
			target = st
			glog.V(3).Infof("PurchaseApp: Found state in memory for user=%s app=%s productID=%s", userID, realAppID, productID)
		} else {
			// Try LoadState (will check Redis and load to memory)
			glog.V(3).Infof("PurchaseApp: State not in memory, trying LoadState from Redis. Error: %v", err)
			if st, err := globalStateMachine.LoadState(userID, realAppID, productID); err == nil {
				target = st
				glog.V(3).Infof("PurchaseApp: Found state in Redis and loaded to memory for user=%s app=%s productID=%s", userID, realAppID, productID)
			} else {
				glog.Errorf("PurchaseApp: State not found in Redis either. Error: %v", err)
			}
		}
	}
	...
	if target == nil {
		...
		glog.V(3).Infof("PurchaseApp: Payment state not found for user=%s app=%s productID=%s. Attempting to create state via preprocessing.", userID, realAppID, productID)
		...
		if err != nil {
			glog.Errorf("PurchaseApp: Failed to preprocess payment data: %v", err)
		...
		if st, err := globalStateMachine.LoadState(userID, realAppID, productID); err == nil {
			target = st
			glog.V(3).Infof("PurchaseApp: Successfully created and loaded state after preprocessing")
		} else {
			glog.Errorf("PurchaseApp: State still not found after preprocessing. Error: %v", err)
	...
	if latest != nil && sourceID != "" && latest.SourceID != sourceID {
		glog.V(3).Infof("PurchaseApp: Correcting SourceID mismatch - state has %s, request has %s", latest.SourceID, sourceID)
		...
		if updated, err := globalStateMachine.getState(userID, realAppID, productID); err == nil && updated != nil {
			latest = updated
			glog.V(3).Infof("PurchaseApp: SourceID corrected to %s", latest.SourceID)
		}
	}
	...
	if latest != nil &&
		...
		glog.V(3).Infof("PurchaseApp: Triggering payment state sync after start_payment event (signature status=%s)", latest.SignatureStatus)
		...
		if latest.DeveloperSync == DeveloperSyncCompleted && latest.VC != "" {
			...
			glog.V(3).Infof("PurchaseApp: Skip triggerPaymentStateSync because signature is still pending (status=%s)", latest.SignatureStatus)
		}
	}
	...
	if err == nil && response != nil {
		if status, ok := response["status"]; ok {
			glog.V(3).Infof("PurchaseApp: buildPurchaseResponse returned status=%v for user=%s app=%s product=%s", status, userID, realAppID, productID)
		} else {
			glog.V(3).Infof("PurchaseApp: buildPurchaseResponse returned payload without status for user=%s app=%s product=%s", userID, realAppID, productID)
		}
	} else if err != nil {
		glog.Errorf("PurchaseApp: buildPurchaseResponse failed for user=%s app=%s product=%s err=%v", userID, realAppID, productID, err)
	}
	return response, err
}
...
func GetPaymentStatus(userID, appID, sourceID, xForwardedHost string, appInfo *types.AppInfo) (*PaymentStatusResult, error) {
	...
	if globalStateMachine != nil && xForwardedHost != "" && state.XForwardedHost == "" {
		if err := globalStateMachine.updateState(state.GetKey(), func(s *PaymentState) error {
			...
			glog.Errorf("GetPaymentStatus: Failed to update XForwardedHost: %v", err)
...
func ProcessSignatureSubmission(jws, signBody, user, xForwardedHost string) error {
	...
	if err != nil {
		glog.Errorf("Failed to parse productID from signBody: %v", err)
	...
	if state == nil {
		glog.Errorf("PaymentState not found for user %s and productID %s", user, productID)
	...
	if err := globalStateMachine.updateState(state.GetKey(), func(s *PaymentState) error {
		...
		glog.Errorf("Failed to update signature status: %v", err)
	...
	if shouldNotifyFrontend && globalStateMachine.dataSender != nil {
		...
		if developerDID == "" {
			glog.Error("Cannot notify frontend payment required: invalid developer DID in state")
		...
		if err := notifyFrontendPaymentRequired(
			...
			glog.Errorf("Failed to notify frontend payment required: %v", err)
...
func HandleFetchSignatureCallback(jws, signBody, user string, signed bool) error {
	glog.V(2).Info("=== Payment State Machine Processing Fetch Signature Callback ===")
	glog.V(2).Infof("JWS: %s", jws)
	glog.V(2).Infof("SignBody: %s", signBody)
	glog.V(2).Infof("User: %s", user)
	glog.V(2).Infof("Signed: %v", signed)

	if globalStateMachine == nil {
		glog.V(3).Info("State machine not initialized, skipping fetch signature callback")
		return nil
	}
	...
	glog.V(3).Info("=== End of Fetch Signature Callback Processing ===")
	return nil
}
...
func StartFrontendPayment(userID, appID, sourceID, productID, xForwardedHost string, appInfo *types.AppInfo, frontendData map[string]interface{}) (map[string]interface{}, error) {
	glog.V(2).Info("=== StartFrontendPayment ===")
	glog.V(2).Infof("User ID: %s", userID)
	glog.V(2).Infof("Source ID: %s", sourceID)
	glog.V(2).Infof("App ID: %s", appID)
	glog.V(2).Infof("X-Forwarded-Host: %s", xForwardedHost)
	...
	if appInfo.AppEntry != nil && appInfo.AppEntry.ID != "" {
		realAppID = appInfo.AppEntry.ID
		glog.V(3).Infof("StartFrontendPayment: Using real app ID from AppEntry: %s (URL param was: %s)", realAppID, appID)
	}

	resolvedProductID := strings.TrimSpace(productID)
	if resolvedProductID != "" {
		glog.V(3).Infof("StartFrontendPayment: Using productID from request: %s", resolvedProductID)
	} else {
		glog.V(3).Infof("StartFrontendPayment: Request missing productID, deriving from app info")
	...
	if err != nil || state == nil {
		glog.Errorf("StartFrontendPayment: State not found in memory, attempting to load from Redis. Err: %v", err)
	...
	// Correct SourceID if it doesn't match the request sourceID
	if latest != nil && sourceID != "" && latest.SourceID != sourceID {
		glog.V(3).Infof("StartFrontendPayment: Correcting SourceID mismatch - state has %s, request has %s", latest.SourceID, sourceID)
		_ = globalStateMachine.updateState(latest.GetKey(), func(s *PaymentState) error {
			s.SourceID = sourceID
			return nil
		})
		...
		if updated, err := globalStateMachine.getState(userID, realAppID, resolvedProductID); err == nil && updated != nil {
			latest = updated
			glog.V(3).Infof("StartFrontendPayment: SourceID corrected to %s", latest.SourceID)
		}
	}
...
func StartPaymentPolling(userID, sourceID, appID, productID, txHash, xForwardedHost string, appInfoLatest *types.AppInfoLatestData) error {
	glog.V(2).Info("=== Starting Payment Polling ===")
	glog.V(2).Infof("User ID: %s", userID)
	glog.V(2).Infof("Source ID: %s", sourceID)
	glog.V(2).Infof("App ID: %s", appID)
	glog.V(2).Infof("Product ID: %s", productID)
	glog.V(2).Infof("TxHash: %s", txHash)
	glog.V(2).Infof("X-Forwarded-Host: %s", xForwardedHost)

	if appInfoLatest != nil && appInfoLatest.RawData != nil {
		if appInfoLatest.RawData.Name != "" {
			glog.V(3).Infof("App Name: %s", appInfoLatest.RawData.Name)
		}
		if appInfoLatest.RawData.Developer != "" {
			glog.V(3).Infof("Developer: %s", appInfoLatest.RawData.Developer)
		}
	}

	// Check if state machine is initialized
	if globalStateMachine == nil {
		glog.V(3).Infof("State machine not initialized, cannot start payment polling")
		return fmt.Errorf("state machine not initialized")
	}
	...
	resolvedProductID := strings.TrimSpace(productID)
	if resolvedProductID != "" {
		glog.V(3).Infof("StartPaymentPolling: Using productID from request: %s", resolvedProductID)
	} else if appInfoLatest != nil && appInfoLatest.AppInfo != nil {
		glog.V(3).Infof("StartPaymentPolling: Request missing productID, deriving from app info")
		resolvedProductID = resolveProductID(appInfoLatest.AppInfo, realAppID)
	} else {
		glog.V(3).Infof("StartPaymentPolling: No productID and no app info, reverting to app ID: %s", realAppID)
	...
	if err != nil {
		glog.Errorf("Failed to get state for polling: %v", err)
	...
	if err := globalStateMachine.updateState(key, func(s *PaymentState) error {
		...
		glog.Errorf("Failed to update state: %v", err)
	...
	if err := globalStateMachine.processEvent(
		...
		glog.Errorf("Failed to process payment_completed event: %v", err)
		return fmt.Errorf("failed to process payment_completed event: %w", err)
	}

	glog.V(3).Infof("Payment polling started for user %s, app %s, product %s", userID, realAppID, resolvedProductID)
	return nil
}
...
func RestorePurchase(userID, appID, sourceID, xForwardedHost string, appInfo *types.AppInfo) (map[string]interface{}, error) {
	glog.V(2).Infof("[RestorePurchase] user=%s app=%s source=%s", userID, appID, sourceID)
	...
	if appInfo.AppEntry != nil && appInfo.AppEntry.ID != "" {
		realAppID = appInfo.AppEntry.ID
		glog.V(3).Infof("RestorePurchase: Using real app ID from AppEntry: %s (URL param was: %s)", realAppID, appID)
	}
	...
	if appInfo.Price != nil && appInfo.Price.Paid != nil {
		if appInfo.Price.Paid.ProductID != "" {
			productID = appInfo.Price.Paid.ProductID
			glog.V(3).Infof("RestorePurchase: Using productID from Price.Paid: %s", productID)
		}
	}
	if productID == "" {
		productID = getProductIDFromAppInfo(appInfo)
		if productID != "" {
			glog.V(3).Infof("RestorePurchase: Using productID from Products: %s", productID)
		}
	}
	if productID == "" {
		// Final fallback: for paid apps without explicit product_id
		productID = realAppID
		glog.V(3).Infof("RestorePurchase: productID not found in price, using real appID as fallback: %s", productID)
	}
	...
	if st, err := globalStateMachine.getState(userID, realAppID, productID); err == nil {
		target = st
		glog.V(3).Infof("RestorePurchase: Found state in memory for user=%s app=%s productID=%s", userID, realAppID, productID)
	} else {
		// Try LoadState (will check Redis and load to memory)
		glog.Errorf("RestorePurchase: State not in memory, trying LoadState from Redis. Error: %v", err)
		if st, err := globalStateMachine.LoadState(userID, realAppID, productID); err == nil {
			target = st
			glog.V(3).Infof("RestorePurchase: Found state in Redis and loaded to memory for user=%s app=%s productID=%s", userID, realAppID, productID)
		} else {
			glog.Errorf("RestorePurchase: State not found in Redis either. Error: %v", err)
		}
	}

	// If state not found, try to create via preprocessing
	if target == nil {
		glog.V(3).Infof("RestorePurchase: Payment state not found for user=%s app=%s productID=%s. Attempting to create state via preprocessing.", userID, realAppID, productID)
		...
		if err != nil {
			glog.Errorf("RestorePurchase: Failed to preprocess payment data: %v", err)
			return nil, fmt.Errorf("failed to create payment state: %w", err)
		}
		...
		if st, err := globalStateMachine.LoadState(userID, realAppID, productID); err == nil {
			target = st
			glog.V(3).Infof("RestorePurchase: Successfully created and loaded state after preprocessing")
		} else {
			glog.Errorf("RestorePurchase: State still not found after preprocessing. Error: %v", err)
			return nil, fmt.Errorf("payment state not found after preprocessing for product '%s'", productID)
		}
	}
	...
	if target != nil && sourceID != "" && target.SourceID != sourceID {
		glog.V(3).Infof("RestorePurchase: Correcting SourceID mismatch - state has %s, request has %s", target.SourceID, sourceID)
		_ = globalStateMachine.updateState(target.GetKey(), func(s *PaymentState) error {
			s.SourceID = sourceID
			return nil
		})
	...
	if latest.SignatureStatus == SignatureRequiredAndSigned && latest.JWS != "" {
		...
		glog.V(3).Infof("RestorePurchase: Signature already available, starting VC polling for user=%s app=%s product=%s", userID, realAppID, productID)
	...
	if latest.SignatureStatus == SignatureRequired || latest.SignatureStatus == SignatureNotEvaluated ||
		latest.SignatureStatus == SignatureRequiredButPending {
		glog.V(3).Infof("RestorePurchase: Signature not available, triggering signature request for user=%s app=%s product=%s", userID, realAppID, productID)
		...
		if globalStateMachine.dataSender != nil {
			...
			if err := notifyFrontendStateUpdate(globalStateMachine.dataSender, latest.UserID, latest.AppID, latest.AppName, latest.SourceID, "signature_required"); err != nil {
				glog.Errorf("Failed to notify frontend signature_required for restore purchase: %v", err)
			}
		}
	...
	if latest.SignatureStatus == SignatureErrorNeedReSign {
		glog.V(3).Infof("RestorePurchase: Signature invalid, triggering re-sign request for user=%s app=%s product=%s", userID, realAppID, productID)
		...
		if globalStateMachine.dataSender != nil {
			...
			// Notify frontend that signature is required for restore purchase
			if err := notifyFrontendStateUpdate(globalStateMachine.dataSender, latest.UserID, latest.AppID, latest.AppName, latest.SourceID, "signature_required"); err != nil {
				glog.Errorf("Failed to notify frontend signature_required for restore purchase re-sign: %v", err)
			}
		}
